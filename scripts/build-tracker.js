...
function parseBuildOutput(output) {
  ...
  lines.forEach((line) => {
    if (line.includes("Compiled successfully")) {
      ...
      if (match) stats.compilationTime = Number.parseFloat(match[1]) * 1000;
    }
    if (line.includes("Finished TypeScript")) {
      const match = line.match(/(\d+\.?\d*)ms/);
      if (match) stats.typescriptTime = Number.parseFloat(match[1]);
    }
    if (line.includes("Collecting page data")) {
      const match = line.match(/(\d+\.?\d*)ms/);
      if (match) stats.pageDataTime = Number.parseFloat(match[1]);
    }
    if (line.includes("Generating static pages")) {
      const match = line.match(/(\d+\.?\d*)ms/);
      if (match) stats.staticGenTime = Number.parseFloat(match[1]);
    }
    if (line.includes("Finalizing page optimization")) {
      const match = line.match(/(\d+\.?\d*)ms/);
      if (match) stats.finalizationTime = Number.parseFloat(match[1]);
    }
  });
...
function logPerformance(stats) {
  ...
  if (!fs.existsSync(logFile)) {
    fs.writeFileSync(
      logFile,
      "timestamp,totalTime,compilationTime,typescriptTime,pageDataTime,staticGenTime,finalizationTime\n",
    );
  }
...
try {
  ...
  if (lines.length > 2) {
    ...
    recentBuilds.forEach((line, index) => {
      ...
      const timeStr = `${(Number.parseFloat(totalTime) / 1000).toFixed(2)}s`;
